...
This is how the component Brand Bar and the json work together to render a template within pattern lab.


### data.json:
Holds all default properties.

### 07a-profile-communities.json:

This holds all component overrides that are template specific. Example below:
...
`lifestyle`, `communities`, and `living` designate what logo is shown.

`first-color` designates the left most bar colour.

`third-color` designates the right most colour ( and view map colour ).

`view-map-button` indicates if there’s a view map button.

`mapOption` is another component that holds the map. This hides the legend shown on the search results page.


### [Icomoon Documentation](/source/assets/vendor)
### [Image Documentation](/source/assets/images)
### [CSS Documentation](/source/assets/css)
### [JS Documentation](/source/assets/js)
